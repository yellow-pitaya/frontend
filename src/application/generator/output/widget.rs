    ...
    fn update(&mut self, event: Signal) {
        match event {
            ...
            Signal::Form(form) => {
                ...
                self.duty_cycle.emit(::widget::precise::Signal::SetVisible(is_pwm));
...
impl ::relm::Widget for Widget {
    ...
    fn view(relm: &::relm::Relm<Self>, model: Self::Model) -> Self {
        ...
        duty_cycle.emit(::widget::precise::Signal::SetDigits(2));
        duty_cycle.emit(::widget::precise::Signal::SetAdjustement(::gtk::Adjustment::new(
    ...
    fn init_view(&mut self) {
        match self.model.generator.get_form(self.model.source) {
            Ok(form) => self.form.emit(::widget::radio::Signal::Set(form)),
            Err(err) => error!("{}", err),
        };

        match self.model.generator.get_amplitude(self.model.source) {
            Ok(amplitude) => self.amplitude.emit(::widget::precise::Signal::SetValue(amplitude as f64)),
            Err(err) => error!("{}", err),
        };

        match self.model.generator.get_offset(self.model.source) {
            Ok(offset) => self.offset.emit(::widget::precise::Signal::SetValue(offset as f64)),
            Err(err) => error!("{}", err),
        };

        match self.model.generator.get_duty_cycle(self.model.source) {
            Ok(duty_cycle) => self.duty_cycle.emit(::widget::precise::Signal::SetValue(duty_cycle as f64)),
            Err(err) => error!("{}", err),
        };

        match self.model.generator.get_frequency(self.model.source) {
            Ok(frequency) => self.frequency.emit(::widget::precise::Signal::SetValue(frequency as f64)),
            Err(err) => error!("{}", err),
        };
    }
}
